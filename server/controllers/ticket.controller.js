...
.catch(err => {
    res.status(500).send({
        message:
        err.message || "Some error occured while creating the Ticket."
        });
    });
};
...
exports.deleteAll = (req, res) => {
    ...
    .catch(err => {
        res.status(500).send({
            message:
            err.message || "Some error occured while removing all tickets"
        });
    });
};
...
exports.findAllPublished = (req, res) => {
    ...
    .catch(err => {
        res.status(500).send({
            message:
            err.message || "Some error occoured while retriving tickets."
        });
    });
};
